...
async function chkConnect( pDB, pDBSN ) {                           // .(10928.02.4 Beg RAM Added).(21208.01.1 RAM It's an Object)
    try { 
...
//          console.log( `  dbConnect[ 9]    Can connect to DBSN: ${aDBSN}.` )
            ...
            console.log( `  dbConnect[10] ** Error: Can't connect to DBSN: ${pDBSN.DBSN}. **`, err ) // .(21208.01.2)
            process.exit()
...
//       aFile1       = (aConfigFile == aDir + aFile2) ? '' : aFile1                                        //#.(10220.04.2 Get rid of 'none' if my.config.json is used ).(10325.06.4 ???)
    ...
    var  pJSON1 = { };   pJSON1[ aDBSN ] = pJSON; pJSON = pJSON1                                            // .(10220.06.1 If there is no DBSN)
     ...
     var aDBSN  = (mDBSNs.length == 1)  ? mDBSNs[0] : aFile1 || mDBSNs[0]                                // .(21208.01.2 RAM Needs work if aDBSN is not in local configFile)
